...
*How does wolf change with time? Might be a function of wolf population and deer population. In dynamic models more than one thing can change through time. Growth rate, carrying capacity, etc. change through time.*
...
-   dams emissions of CO2
    ...
    *Why do dams emit CO2 in the first place? This might be things like amount of sediment or bio-accumulation. Water temp and level might be inputs.*

    *A regression equation could be used to model this*
...
-   groundwater
    ...
    *If you just want to know how wildfires degrade water quality that is one model. Another is how fire travels through space and you measure this to understand the system.*
...
# Stepping through caused a small error to occur because as population changes derivative changes
```
...
*Work with numerical approach coded in R, this solver is just a way to do an integration*

*You have to code up derivative as s function*
...
ggplot(as.data.frame(Ptime), aes(year, Pop)) + geom_point() + labs(y = "Pop", x = "Years")
...
![](images/diffusion/diffusion.001.jpeg)  
