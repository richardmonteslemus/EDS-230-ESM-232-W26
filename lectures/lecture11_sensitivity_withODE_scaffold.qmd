...
# test with specific r and K values (r=0.01, K=150) and Pinitial=3 # just do 10 time steps
  ...
  p_wrapper(r = 0.01, K = 150, Pinitial = 3, simtimes = seq(from = 1, to = 10), odefunc = dpopgrowth, 
            metricfunc = compute_pop_metrics, metricpars = metricpars)
...
ggplot(allresp, aes(K, threshyear, col = r)) + geom_point() + labs(y = "Year that the threshold is reached", x = "K")

# THe higher the carrying the capacity the earlier the threhhold is reached. r for rate is also showing higher values of r associated with lower values for year threshold is reached. 
