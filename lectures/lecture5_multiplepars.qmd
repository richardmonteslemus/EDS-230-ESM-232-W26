...
-   can't have negative efficiency;
    ...
    -   An efficiency cant go lower than some point because it wouldn't be marketable
...
-   sometimes we think of uncertainty as percent deviation from our best guess e.g., we think efficiency is 0.2 but could be ±15%
    ...
    -   We can still do a sensitivity analysis by varying it this way. This is the least effective way to actually do it.
...
1.  Reduce output dimensionality - summarize output e.g mean annual electricity - one value per parameter set
    ...
    1.  If you have output varying in space and time is there a way to captures those changes? Yes like like mean for example to address time.
...
2.  To deal with multiple parameter, add a "key" to identify parameter set - e.g. run number .e.g we will use a parameter id to do this
    ...
    1.  Add identifier for parameter and link to model run
...
Lets say we want to model power generated by a reservoir.

Stream model -\> Reservoir model -\> height -\> power model -\> power

From power model -\> flow rate which re influences model

Fully coupled model: Models exchange inputs and outputs and there are feedback systems.
